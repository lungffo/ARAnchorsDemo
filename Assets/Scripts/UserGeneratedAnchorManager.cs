...
/// </summary>
public class UserGeneratedAnchorManager : MonoBehaviour
{
    ...
    public TMP_InputField anchorContentText;
    ...
    public static List<CinchDBRecord> savedAnchorRecords;
    ...
    public void ProcessStageUpdate()
    {
        ...
        anchorPlaceholder.GetComponentInChildren<TextMeshProUGUI>().text = anchorContentText.text;

        // Only allow the user to submit the data if they have given the anchor a name.
        if (currentStage == 1)
        {
            anchorDataConfirmButton.interactable = anchorContentText.text.Length > 0;
        }
    ...
    protected virtual void OnAnchorLocated(object sender, AnchorLocatedEventArgs args)
    {
        UnityDispatcher.InvokeOnAppThread(() =>
        {
            ...
            foreach (CinchDBRecord record in savedAnchorRecords)
            {
                if (record.Columns[0] == args.Anchor.Identifier)
                {
                    Handheld.Vibrate();
                    GameObject go = Instantiate(anchorPlaceholder, anchorPose.position, anchorPose.rotation);
                    go.SetActive(true);
                    go.GetComponentInChildren<TextMeshProUGUI>().text = record.Columns[4];
                    if (go.GetComponent<CloudNativeAnchor>() != null)
                        Destroy(go.GetComponent<CloudNativeAnchor>());
    ...
    {
        savedAnchorRecords = await anchorDatabase.GetAllRecords();
        ...
        // Search for all anchors stored in the database.
        List<string> anchorsToFind = new List<string>();
        foreach (CinchDBRecord record in savedAnchorRecords)
        {
            anchorsToFind.Add(record.Columns[0]);
        }
    ...
    public async void UploadAnchorToCloud()
    {
        ...
        try
        {
            ...
            GameObject go = Instantiate(anchorPlaceholder, anchorPlaceholder.transform.position, anchorPlaceholder.transform.rotation);
            go.SetActive(true);
            cloudNativeAnchor = anchorPlaceholder.AddComponent<CloudNativeAnchor>();
        } catch { }
        currentStage = 0;
    ...
    public async Task SaveAnchorToDatabase()
    {
        ...
        record.Columns[1] = "UserMade_ " + UnityEngine.Random.Range(10000, 99999);
        record.Columns[2] = "";
        record.Columns[3] = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
        record.Columns[4] = anchorContentText.text;
